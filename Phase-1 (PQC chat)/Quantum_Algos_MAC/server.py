...
if not TARGET_RECEIVER_IP:
    # Use localhost as fallback if not set to prevent immediate crash, 
    # but print warning.
    print("⚠️ TARGET_RECEIVER_IP not set in .env. Defaulting to 127.0.0.1")
    TARGET_RECEIVER_IP = "127.0.0.1"
...
async def lifespan(app: FastAPI):
    ...
    print("\n" + "="*60)
    print(f"🚀 QUANTUM SAFE SERVER STARTED on http://{local_ip}:{SERVER_PORT}")
    print("="*60)
    print("👉 Send this to your friend (The Encapsulator):")
    print(f"""
curl -X POST http://{local_ip}:{SERVER_PORT}/decapsulate \\
...
""")
    ...
    # We can use the global vars now
    ...
    if role != "INITIATOR":
        ...
        if workflow_lock.acquire(timeout=10):
            try:
                keys = phase1.run_workflow()
            finally:
                workflow_lock.release()
        
        # 2. Try to Send to Receiver (Background Thread)
...
app = FastAPI(title="Quantum Safe KeyGen Server", lifespan=lifespan)
...
def handle_offer(payload: dict = Body(...), request: fastapi.Request = None):
    ...
    try:
        ...
        with phase1.ManualKyberKeyGen(phase1.KYBER_ALG) as client_kem:
            pk_bytes = bytes.fromhex(kyber_pk)
            ciphertext, shared_secret = client_kem.encapsulate(pk_bytes)
...
def chat_input_loop():
    ...
    print("\n⏳ WAITING FOR HANDSHAKE TO COMPLETE...", end="", flush=True)
    
    # BLOCKING WAIT for Shared Secret
    while not CURRENT_SHARED_SECRET:
        time.sleep(1)
        
    print("\n\n" + "="*50)
    print("💬 QUANTUM CHAT ACTIVE. Type a message and press Enter.")
    print("="*50 + "\n")
